...
import {
  ...
  RefreshCw, Link2, CheckCircle2, XCircle, Plug, Plus, X as XIcon,
} from 'lucide-react';
...
const IntegrationsTab = () => {
  ...
  const { accounts, syncingEmail, addAccount, removeAccount, syncAccount, syncAll } =
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      toast.success(`Connected ${acc.email}`);
    } catch (e) {
  ...
  const handleSyncOne = async (email: string) => {
    setError('');
    try {
      const count = await syncAccount(email, syncWeeks);
      toast.success(`${email}: imported ${count} new event${count !== 1 ? 's' : ''}`);
    } catch (e) {
  ...
  const handleSyncAll = async () => {
    setError('');
    try {
      const results = await syncAll(syncWeeks);
      const total = results.reduce((sum, r) => sum + r.imported, 0);
      const failed = results.filter((r) => r.error).length;
      if (failed === 0) {
        toast.success(`Imported ${total} new event${total !== 1 ? 's' : ''} across ${results.length} account${results.length !== 1 ? 's' : ''}`);
      } else {
        toast.error(`Imported ${total}, but ${failed} account${failed !== 1 ? 's' : ''} failed`);
      }
    } catch (e) {
  ...
  const handleRemove = (email: string) => {
    if (!confirm(`Disconnect ${email}? Already-imported events stay in your calendar.`)) return;
    removeAccount(email);
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-xs text-white/40">
          Link multiple Google accounts to see and manage events from all of them in one calendar.
        </p>
      </div>
      ...
      {!hasClientId && (
        <div className="p-4 rounded-xl border border-yellow-500/25 bg-yellow-500/8 space-y-3">
          ...
          <p className="text-xs text-white/50 leading-relaxed">
            Add your Google OAuth client ID to <code className="text-white/70 bg-[#1a1a1a] px-1.5 py-0.5 rounded">.env.local</code> as
            <code className="text-[#00d4ff] text-[10px] ml-1">NEXT_PUBLIC_GOOGLE_CLIENT_ID</code> and enable the Calendar API in Google Cloud Console.
          </p>
        </div>
      )}
      ...
      <div className="space-y-2">
        {accounts.length === 0 ? (
          <div className="p-6 rounded-2xl border border-dashed border-[#333333] bg-[#0a0a0a] text-center">
            ...
            <p className="text-sm text-white font-medium mb-1">No accounts connected</p>
            <p className="text-xs text-white/40">Connect a Google account to import its calendar events.</p>
          </div>
        ) : (
          accounts.map((acc) => (
            <div key={acc.email} className="p-4 rounded-2xl border border-[#333333] bg-[#111111]">
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-white truncate">{acc.email}</p>
                  <div className="flex items-center gap-1.5 mt-0.5">
                    <CheckCircle2 className="w-3 h-3 text-green-400 flex-shrink-0" />
                    <span className="text-[11px] text-white/50 truncate">
                      {acc.lastSync ? `Last synced ${fmtTime(acc.lastSync)}` : 'Not synced yet'}
                      {acc.importCount > 0 && ` · +${acc.importCount}`}
                    </span>
                  </div>
                </div>
                <button
                  onClick={() => handleSyncOne(acc.email)}
                  disabled={syncingEmail === acc.email}
                  className="p-2 rounded-lg text-white/50 hover:text-white hover:bg-[#1a1a1a] transition-colors disabled:opacity-40"
                  title="Sync this account"
                >
                  <RefreshCw className={cn('w-4 h-4', syncingEmail === acc.email && 'animate-spin')} />
                </button>
                <button
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        <NeonButton
          size="sm"
          glow={hasClientId}
          disabled={!hasClientId || adding}
          loading={adding}
          icon={<Plus className="w-3.5 h-3.5" />}
          onClick={handleAdd}
          className="flex-1"
        >
          {adding ? 'Connecting…' : 'Add Google account'}
        </NeonButton>
        {accounts.length > 0 && (
          <NeonButton
            size="sm"
      ...
      <div className="p-3 rounded-xl bg-[#111111] border border-[#222222]">
        <p className="text-xs text-white/30 leading-relaxed">
          Each account is authorized separately. Duplicate events (same title and time) are skipped on sync, so you can safely re-sync.
